...
class TestSecretsCli(unittest.TestCase):
    ...
    def test_keygen(self):
        output = self.run_command(['secret', 'keygen']).strip()
    ...
    def test_set_get_list_delete(self):
        output = self.run_command(
            ['secret', 'set', 'gitlab-token', 'secret-value', '--key', self.key]
        )
        self.assertEqual(output.strip(), 'secret set: gitlab-token')

        output = self.run_command(
            ['secret', 'get', 'gitlab-token', '--key', self.key]
        )
        self.assertEqual(output.strip(), 'secret-value')

        output = self.run_command(['secret', 'list', '--key', self.key])
        self.assertEqual(output.strip(), 'gitlab-token')

        output = self.run_command(
            ['secret', 'ls', '--key', self.key]
        )
        self.assertEqual(output.strip(), 'gitlab-token')

        output = self.run_command(
            ['secret', 'delete', 'gitlab-token', '--key', self.key]
        )
        self.assertEqual(output.strip(), 'secret deleted: gitlab-token')

        with self.assertRaises(SystemExit) as ctx:
            self.run_command(['secret', 'get', 'gitlab-token', '--key', self.key])
        self.assertIn('secret not found', str(ctx.exception))

    def test_set_from_stdin(self):
        output = self.run_command(
            ['secret', 'add', 'token', '-', '--key', self.key],
            stdin='from-stdin\n',
        )
        self.assertEqual(output.strip(), 'secret set: token')
        output = self.run_command(['secret', 'get', 'token', '--key', self.key])
    ...
    def test_missing_repository(self):
        ...
        try:
            os.chdir(empty)
            with self.assertRaises(SystemExit) as ctx:
                self.run_command(['secret', 'list', '--key', self.key])
    ...
    def test_key_file(self):
        ...
        self.run_command(
            ['secret', 'set', 'a', 'b', '--key-file', key_path]
        )
        output = self.run_command(
            ['secret', 'get', 'a', '--key-file', key_path]
        )
        self.assertEqual(output.strip(), 'b')

    def test_delete_unknown(self):
        with self.assertRaises(SystemExit) as ctx:
            self.run_command(['secret', 'rm', 'missing', '--key', self.key])
        self.assertIn('unknown secret', str(ctx.exception))

    def test_status_empty(self):
        output = self.run_command(['secret', 'status', '--key', self.key])
    ...
    def test_status_set_vs_missing(self):
        ...
        output = self.run_command([
            'secret', 'status', 'gitlab-token', 'db-password', '--key', self.key
        ])
    ...
    def test_resolve_success(self):
        ...
        output = self.run_command([
            'secret', 'resolve', 'gitlab-token', '--key', self.key
        ])
    ...
    def test_resolve_missing_hard_fail(self):
        ...
        with self.assertRaises(SystemExit) as ctx:
            self.run_command([
                'secret', 'resolve', 'gitlab-token', 'db-password', '--key', self.key
            ])
    ...
    def test_resolve_prompt_sets_missing(self):
        ...
        with patch('getpass.getpass', return_value='pw-value'):
            output = self.run_command([
                'secret', 'resolve', 'gitlab-token', 'db-password',
                '--prompt', '--key', self.key
            ])
        self.assertEqual(output, '')
        output = self.run_command([
            'secret', 'get', 'db-password', '--key', self.key
        ])
    ...
    def test_clear_requires_force(self):
        ...
        with self.assertRaises(SystemExit) as ctx:
            self.run_command(['secret', 'clear', '--key', self.key])
    ...
    def test_clear_force(self):
        ...
        output = self.run_command(['secret', 'clear', '--force', '--key', self.key])
