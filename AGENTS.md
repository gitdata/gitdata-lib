...
- CLI: `gitdata secrets` (`list`/`ls`, `get`, `set`/`add`, `delete`/`rm`, `keygen`). Key from `--key`, `--key-file`, `GITDATA_ENCRYPTION_KEY`, or `/run/secrets/gitdata_encryption_key`. Never store the key in `.gitdata`.
