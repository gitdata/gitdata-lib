...
- CLI: `gitdata secret` (`list`/`ls`, `get`, `set`/`add`, `delete`/`rm`, `status`, `resolve`, `clear`, `keygen`). `gitdata secrets` is an alias. Key from `--key`, `--key-file`, `GITDATA_ENCRYPTION_KEY`, or `/run/secrets/gitdata_encryption_key`. Never store the key in `.gitdata`.
