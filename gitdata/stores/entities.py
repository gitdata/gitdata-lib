"""
    gitdata.stores.entity

    Entity Store

    An EntityStore is a high level class that provides a standard interface
    for storing and working with Records but using a universal data model
    schema rather than a static data model schmea.  This allows fast prototyping
    and interating of data based applications with reasonable performance
    while the application is small ( < 1,000,000 records ).

    Because the interface is consistent with the other stores in the gitdata.stores
    subpackage, data models can be easly switched over to a more appropriate data
    storage technology once the data model is known without the cost of significant
    applicatino code changes.
...
Record = gitdata.utils.Record
RecordList = gitdata.utils.RecordList
...
def entify(rs, storage):
    ...
    for _, _, row_id, attribute, datatype, value in rs:
        ...
        if datatype == 'str':
            ...
            value = gitdata.json.loads(value)

        elif datatype == 'tuple':
            value = tuple(gitdata.json.loads(value))
    ...
    return RecordList(entities.values())
...
class EntityStore(Store):
    ...
    def __init__(self, db=None, klass=dict, kind=None):
        ...
        self.kind = kind or type(klass) == str and klass or gitdata.utils.kind(klass())
        self.id_name = '_id'
    ...
    def put(self, entity):
        ...
        for n, atype in enumerate(datatypes):
            if atype not in valid_types:
                ...
                raise UnsupportedTypeException(msg % (atype, keys[n]))
...
def store_of(klass, db=None, name=None):
    ...
    an entity kind.  If a gitdata.Model is provided the actual
    entity kind is derived from the class name.  If the entity
    kind is provivded as a string then it's taken as-is.
    ...
    >>> class Thing(Record): pass
    >>> things = store_of(Thing, db, 'person')
